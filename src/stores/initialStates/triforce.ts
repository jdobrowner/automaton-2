...
function getValueUp(n: number, m: number) {
  return triangleUp(n, m, apex1, 2) || triangleUp(n, m, apex2, 1) || triangleUp(n, m, apex3, 3);
}

function getValueDown(n: number, m: number) {
  return (
    triangleDown(n, m, apex1, 2) || triangleDown(n, m, apex2, 1) || triangleDown(n, m, apex3, 3)
  );
}

function triangleDown(n: number, m: number, apex: ApexType, color: number) {
...
function triangleUp(n: number, m: number, apex: ApexType, color: number) {
  if (m >= apex.y && m < apex.y + triangleHeight) {
    if (n >= apex.x - triangleHeight / 2 && n < apex.x + triangleHeight / 2) {
      const row = m - apex.y;
